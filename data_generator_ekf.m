...
fileID = fopen('obj_pose-laser-radar-synthetic-ekf-input.txt','w');
[nrows,ncols] = size(AB);
...
for row = 1:nrows
    ...
    if( AB{row,1} == 'L' &&  mod(row,4) == 1 )
        ...
        %when 
        %estimations
        fprintf(fileID,'%s\t%.4f\t%.4f\t%.0f\t',AB{row,1:ncols-1});
        %ground truth speed and position
        fprintf(fileID,'%n\t%.4f\t%.4f\t%.4f\t',GTT(row_gt, [1 2 6 7]));
    ...
    elseif( AB{row,1} == 'R' &&  mod(row,4) == 0 )
        ...
        %estimations
        fprintf(fileID,'%s\t%.4f\t%.4f\t%.4f\t%.0f\t',AB{row,:});
        %ground truth speed and position
        fprintf(fileID,'%.4f\t%.4f\t%.4f\t%.4f\t',GTT(row_gt, [1 2 6 7]));
         %yaw and yaw_rate
